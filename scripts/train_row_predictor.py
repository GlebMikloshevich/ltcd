...
from ltcd.losses.row_losses import get_row_loss_function
...
def train_one_epoch(
    ...
    for images, keypoints in tqdm(loader, desc="Training"):
        ...
        # Starting point is first keypoint of the row
        start_xy = keypoints[:, 0:1, :]  # [B, 1, 2]

        optimizer.zero_grad()

        with torch.amp.autocast(device_type="cuda", enabled=(device == "cuda")):
            pred_keypoints = model(
                image=images,
                start_xy=start_xy,
                teacher_forcing_points=keypoints,
            )

            loss_dict = criterion(pred_keypoints, keypoints)
            loss = loss_dict["loss"]
        ...
        running_mse += loss_dict.get("mse_loss", loss).item()
        running_line += loss_dict.get("line_loss", torch.tensor(0.0)).item()
        running_spacing += loss_dict.get("spacing_loss", torch.tensor(0.0)).item()

    n = len(loader)
    return {
        "loss": running_loss / n,
        "mse_loss": running_mse / n,
        "line_loss": running_line / n,
        "spacing_loss": running_spacing / n,
    }
...
def evaluate(
    ...
    for images, keypoints in tqdm(loader, desc="Evaluating"):
        ...
        start_xy = keypoints[:, 0:1, :]

        pred_keypoints = model(
            image=images,
            start_xy=start_xy,
            teacher_forcing_points=None,
        )

        loss_dict = criterion(pred_keypoints, keypoints)

        running_loss += loss_dict["loss"].item()
        running_mse += loss_dict.get("mse_loss", loss_dict["loss"]).item()
        running_line += loss_dict.get("line_loss", torch.tensor(0.0)).item()
        running_spacing += loss_dict.get("spacing_loss", torch.tensor(0.0)).item()

        # Compute mean distance error (in normalized coords)
        dist = torch.norm(pred_keypoints - keypoints, dim=-1).mean()
...
@hydra.main(version_base=None, config_path="../confs", config_name="train_row_predictor")
def main(cfg: DictConfig) -> None:
    ...
    with mlflow.start_run(run_name=cfg.run_name):
        ...
        model = LSTMRowPredictor(
            hidden_size=cfg.params.hidden_size,
            num_cols=cfg.params.num_cols,
            num_lstm_layers=cfg.params.num_lstm_layers,
        ...
        loss_type = getattr(cfg.params, "loss_type", "combined")
        print(f"Using loss type: {loss_type}")

        loss_kwargs = {}
        if loss_type == "combined":
            loss_kwargs = {
                "mse_weight": cfg.params.mse_weight,
                "line_weight": cfg.params.line_weight,
            }
        elif loss_type == "full":
            loss_kwargs = {
                "mse_weight": cfg.params.mse_weight,
                "line_weight": cfg.params.line_weight,
                "spacing_weight": cfg.params.spacing_weight,
            }

        criterion = get_row_loss_function(loss_type, **loss_kwargs)
        print(f"Loss configuration: {loss_kwargs}")

        weight_decay = getattr(cfg.params, "weight_decay", 1e-4)
        ...
        if scheduler_type == "cosine":
            ...
            step_size = getattr(cfg.params, "step_size", 10)
        ...
        for epoch in range(cfg.params.epochs):
            ...
            mlflow.log_metrics(
                {
                    ...
                    "train_line_loss": train_metrics["line_loss"],
                    "train_spacing_loss": train_metrics["spacing_loss"],
                    "val_loss": val_metrics["loss"],
                    "val_mse_loss": val_metrics["mse_loss"],
                    "val_line_loss": val_metrics["line_loss"],
                    "val_spacing_loss": val_metrics["spacing_loss"],
            ...
            print(f"  MSE: {train_metrics['mse_loss']:.6f} -> {val_metrics['mse_loss']:.6f}")
            print(f"  Line: {train_metrics['line_loss']:.6f} -> {val_metrics['line_loss']:.6f}")
            if train_metrics["spacing_loss"] > 0:
                print(f"  Spacing: {train_metrics['spacing_loss']:.6f} -> {val_metrics['spacing_loss']:.6f}")
